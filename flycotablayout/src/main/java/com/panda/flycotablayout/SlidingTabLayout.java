...
import com.panda.flycotablayout.utils.UnreadMsgUtils;
import com.panda.flycotablayout.widget.MsgView;
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (tv_tab_title != null) {
                ...
                MsgView msgView = v.findViewById(R.id.rtv_msg_tip);
                msgView.setSelected(i == mCurrentTab);
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                MsgView msgView = tabView.findViewById(R.id.rtv_msg_tip);
                msgView.setSelected(isSelect);
     ...
     */
    public void showMsg(int position, int num, int layoutType) {
        ...
        if (tipView != null) {
            ...
            setMsgLocation(position, layoutType);
     ...
     * @param num      num小于等于0显示红点,num大于0显示数字
     */
    public void showMsg(int position, String num, int layoutType) {
        if (position >= mTabCount) {
            return;
        }
...
//            UnreadMsgUtils.show(tipView, num);
            tipView.setText(num);
            TextView tvMsg = tabView.findViewById(R.id.tv_msg);
            tvMsg.setText(num);

            if (mInitSetMap.get(position) != null && mInitSetMap.get(position)) {
                if (layoutType == RIGHT_TOP) {
    ...
    public void hideRightTopMsg(int position) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        TextView tv_msg = tabView.findViewById(R.id.tv_msg);
        if (tv_msg != null) {
     ...
     */
    public void setMsgLocation(int position, int layoutType) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        ...
        if (tipView != null) {
            ...
            RelativeLayout.LayoutParams lp1 = (RelativeLayout.LayoutParams) tvMsg.getLayoutParams();
            tv_tab_title.setPadding((int) mTabPadding, 0, 0, 0);
            //lp.topMargin = mHeight > 0 ? (int) (mHeight - textHeight) / 2 - dp2px(bottomPadding) : 0;
            if (layoutType == RIGHT_TOP) {
                tvMsg.setVisibility(VISIBLE);
                int tabW = tv_tab_title.getRight() - tv_tab_title.getLeft();
                if (tabW > 0) {
                    lp.leftMargin = tabW - (int) mTabPadding;
                    //lp.leftMargin = mTabWidth >= 0 ? (int) (mTabWidth / 2 + textWidth / 2 + dp2px(leftPadding)) : (int) (mTabPadding / 2 + textWidth + dp2px(leftPadding));
                    lp.topMargin = mHeight > 0 ? (int) (mHeight - textHeight) / 2 - (int) (textHeight / 2) : 0;
                    ...
                    tv_tab_title.post(() -> {
                        int newTabW = tv_tab_title.getRight() - tv_tab_title.getLeft();
                        lp.leftMargin = newTabW;
                        lp.topMargin = mHeight > 0 ? (int) (mHeight - textHeight) / 2 - (int) (textHeight / 2) : 0;
                        tipView.setLayoutParams(lp);
                        lp1.leftMargin = lp.leftMargin;
                        lp1.topMargin = lp.topMargin;
                ...
                tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize);
                tv_tab_title.setPadding((int) mTabPadding, 0, 0, 0);
                tipView.setPadding(0, 0, (int) mTabPadding, 0);
